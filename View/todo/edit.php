        ...
        <div class="row">
          <div class="col-lg-12">
            ...
            <div class="card card-primary">
              ...
              <input type="hidden" id="todoID" class="form-control" value="<?= $data['todoID'] ?>">
                <div class="card-body">
                <?php
                // test($data);
                ...
                ?>
                  <div class="form-group">
                      ...
                      <select class="form-control" name="category_id" id="category_id" >
                           ...
                           <?php foreach($data['categories'] as $key => $value){ ?>
                               <option <?= $data['categoriesID'] == $value['categoriesID'] ? 'selected' : null ; ?> value="<?= $value['categoriesID'] ?>"><?= $value['categoriesTitle'] ?></option>
                           <?php } ?>
                      </select>     
                  ...
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="title" id="title" value="<?= $data['todoTitle'] ?>" >
                  </div>
                  <div class="form-group">
                    <label for="description">Açıklama</label>
                    <input type="text" class="form-control" name="description" id="description" value="<?= $data['todoDescription'] ?>">
                  </div>
                  <div class="form-group">
                    <label for="status">Durum</label>
                    <select class="form-control" name="status" id="status">
                          <option value="p" <?= $data['todoStatus'] == 'p' ? 'selected' : null ; ?> >Pasif</option>
                          <option value="a" <?= $data['todoStatus'] == 'a' ? 'selected' : null ; ?> >Aktif</option>
                          <option value="s" <?= $data['todoStatus'] == 's' ? 'selected' : null ; ?> >Süreçte</option>
                      </select>
                  </div>
                  <div class="form-group">
                    <label for="range">İlerleme</label>
                    <input type="range" class="form-control" name="range" id="range" min="0" max="100" value="<?= $data['todoProgress'] ?>">
                  </div>
                  <div class="form-group">
                    <label for="color">Renk seçiniz</label>
                    <input type="color" class="form-control" name="color" id="color" value="<?= $data['todoColor'] ?>" >
                  </div>
                  <div class="form-group">
                    <label for="start_date">Başlangıç tarihi</label>
                   <div class="row">

                   <input type="date" class="form-control col-8" name="start_date" id="start_date" value="<?= date('Y-m-d',strtotime($data['todoStartDate'])) ?>">
                   <input type="time" class="form-control col-4" name="start_time" id="start_time" value="<?= date('H:i',strtotime($data['todoStartDate'])) ?>">
                   </div>
                  </div>
                  <div class="form-group">
                    <label for="finish_date">Bitiş tarihi</label>
                    <div class="row">
                    <input type="date" class="form-control col-8" name="finish_date" id="finish_date" value="<?= date('Y-m-d',strtotime($data['todoEndDate'])) ?>">
                   <input type="time" class="form-control col-4" name="finish_time" id="finish_time" value="<?= date('H:i',strtotime($data['todoEndDate'])) ?>">
                   </div>
                  </div>
                </div>
                <!-- /.card-body -->
